...
export default { title: "Google" };

export const googleApp = props => {
    ...
    if (prifinaID.current !== "") {
      s3Results.current = GRAPHQL.graphql({
        ...
        next: data => {
          ...
          let file = "";
          if (s3.key.indexOf("google-drive") > -1) {
            file = prifinaID.current + "/google-drive/" + fname;
          } else {
            file = prifinaID.current + "/" + fname;
          }
          if (s3.size > 0) {
          ...
          //console.log("s3 ", s3.key, s3.key.split("/"));

          /*
          value:
data:
s3Results:
data: "{"status":"OK","s3":{"bucket":"prifina-app-data-dev","key":"private/us-east-1%3Ab456b38c-c711-46b7-b51f-03230267c132/a277d2fbbca863b140a62fd466bf482829c4/user.jpg"}}"
id: "a277d2fbbca863b140a62fd466bf482829c4"
__proto__: Object
*/
        },
        error: err => {
      ...
      s3Results.current = clientHandler.current
        .subscribe({
          query: gql(s3Notification),
          variables: { id: prifinaID.current },
        })
        .subscribe({
          next: res => {
...
*/
      Storage.list(prifinaID.current + "/", { level: "private" })
        .then(result => {
          console.log(result);
          if (result.length > 0) {
            let existingImages = [];
...
*/
    ...
    try {
      console.log("FILE ", file, Object.keys(file), typeof file.name);

      if (typeof file.name !== "undefined") {
        let metaData = {
          "alt-name": file.name,
          "prifina-id": prifinaID.current,
        };
        await Storage.put(prifinaID.current + "/" + file.name, file, {
          level: "private",
