...
const PhoneVerification = ({ invalidLink, ...props }) => {
  ...
  const checkInput = (code = null, enterPressed = false) => {
    const validateCode = code || inputCode.current.value;
    const checkResult = onlyDigitChars(validateCode);

    let validCode = true;
    if (!checkResult && validateCode.length > 0) {
    ...
    if ((enterPressed && validateCode.length !== 6) || (!enterPressed && validateCode.length > 6)) {
          ...
          <Box width="169px">
            <IconField>
              ...
              <IconField.InputField
                ...
                onKeyDown={e => {
                  if (e.key === "Enter") {
                    checkInput(verificationFields.verificationCode, true);
                  }
                }}
                ref={inputCode}
