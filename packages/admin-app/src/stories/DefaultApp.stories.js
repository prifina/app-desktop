...
import { getCognitoUserDetails, getCognitoUserCount, getCognitoMetrics } from "../lib/queries";
...
const DefaultApp = ({ appSyncClient, prifinaID, GraphQLClient }) => {
  ...
  useEffect(() => {
    async function onLoad() {
      ...
      const test = await appSyncClient.query({
        query: gql(getCognitoMetrics),
        variables: {},
      });
      */
      /*
      const test = await appSyncClient.query({
        query: gql(getCognitoUserCount),
        variables: { "poolID": "eu-west-1_uwy3R12FM" },
      });
      */

      const test = await appSyncClient.query({
        query: gql(getCognitoUserDetails),
        variables: { "attrName": "username", "attrValue": "test-user" },
      });


      console.log("TEST ", test);
      /*
      09fa06d8-c08d-403e-a4ee-66e0b517b105 GraphQL Query: query cognitoUserDetails($attrName: String!, $attrValue: String!) {
  getCognitoUserDetails(attrName: $attrName, attrValue: SattrValue) {
    result
    __typename
  }
}
, Operation: cognitoUserDetails, Variables: 
{
    "attrName": "username",
    "attrValue": "test-user"
}

      893fc55c-9246-4924-bb9e-20398df34458 GraphQL Query: query MyQuery {
        getCognitoUserDetails(attrName: "username", attrValue: "test-user") {
          result
        }
      }
      */
      ...
      try {
        if (login) {
          ...
          */
          ...
          setSettingsReady(true);
          */
          const clientEndpoint = "https://uolvxh3znnh45mzhpn3qk3j3m4.appsync-api.eu-west-1.amazonaws.com/graphql";
          const clientRegion = "eu-west-1"
